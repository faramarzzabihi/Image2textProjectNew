...
class ImageToTextForm(forms.Form):
    ...
    model=forms.ChoiceField(label='Moled', choices=[('Tesseract', 'Tesseract'), ('PadellOCR', 'PadellOCR')], initial='Tesseract')
    #forms.CharField(label='Language', initial='eng', max_length=3, help_text='ISO 639-3 language code')
    post_processing = forms.ChoiceField(label='Do SpellCorrection(It take a while to  be processed)', choices=[('No', 'No'), ('Yes', 'Yes')], initial='No')
